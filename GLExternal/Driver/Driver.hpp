...
public:
    ...
    void WritePointerMemory(const uintptr_t OldPointer, const void* ReplacePointer, size_t size)
    {
        if (!RobloxHandle) {
            return;
        }
        ...
        BOOL Success = WriteProcessMemory(RobloxHandle, reinterpret_cast<LPVOID>(OldPointer), ReplacePointer, size, &Written);
