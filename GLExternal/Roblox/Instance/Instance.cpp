...
void RobloxInstance::SetBytecode(const std::string& Bytecode) { // Credits to xeno for the method
    if (this->ClassName() == "ModuleScript")
    {
        ...
        std::thread([ModuleBytecodePointer, OldBytecode, OldSize]() { // This will reset the bytecode after the bytecode is replaced by the new one
            Sleep(1000);
        ...
        if (BytecodeAllocated)
        {
            ...
            VirtualProtectEx(Driver->GetRobloxHandle(), BytecodeAllocated, sizeof(Bytecode.c_str()), OldProtect, 0);
...
void RobloxInstance::BypassDetections() {
