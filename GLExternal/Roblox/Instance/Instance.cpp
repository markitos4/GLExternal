...
void RobloxInstance::SetBytecode(std::string Bytecode) { 
    ...
    if (this->ClassName() == "ModuleScript")
    {
        ...
        Driver->WritePointerMemory(NewBytecode, Bytecode.c_str(), Bytecode.size());
...
void RobloxInstance::RemoveContextDetections()
{
    if (!this->Valid(true))
        Logger()->Error("Invalid address on RemoveContextDetections");

    Driver->WritePointer(this->Self + Offsets::RequireBypass, 1);
}
void RobloxInstance::RestoreContextDetections()
{
    if (!this->Valid(true))
        Logger()->Error("Invalid address on RestoreContextDetections");

    Driver->WritePointer(this->Self + Offsets::RequireBypass, 0);
}
